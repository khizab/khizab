...
interface MartianProvider extends PluginProvider {
  signTransaction(
    transaction: any,
    options?: any,
...
export function martianWallet() {
  ...
  return createConnector<Provider, Properties>((_config) => ({
    ...
    getProvider() {
      const martian = typeof window !== 'undefined' ? window.martian : undefined
      if (!martian) {
