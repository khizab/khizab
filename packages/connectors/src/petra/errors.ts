export class PetraApiError extends Error {
  constructor(readonly code: number, readonly status: string, message: string) {
    super(message)
    this.status = status
    this.code = code
...
export function codeToError(code: number): PetraApiError {
  return (
    Object.values(PetraApiErrors).find((error) => error.code === code) ??
    PetraApiErrors.INTERNAL_ERROR
  )
}
